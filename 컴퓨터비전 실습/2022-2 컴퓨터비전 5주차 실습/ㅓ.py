...
def calc_M_integral(IxIx_integral, IxIy_integral, IyIy_integral, fsize=5):
    ...
    for row in range(h):
        for col in range(w):
            M[row, col, 0, 0] = calc_local_integral_value(IxIx_integral_pad, left_top=(row, col),
                                                          right_bottom=(row + fsize - 1, col + fsize - 1))
            M[row, col, 0, 1] = calc_local_integral_value(IxIy_integral_pad, left_top=(row, col),
                                                          right_bottom=(row + fsize - 1, col + fsize - 1))
            M[row, col, 1, 1] = calc_local_integral_value(IyIy_integral_pad, left_top=(row, col),
                                                          right_bottom=(row + fsize - 1, col + fsize - 1))
            M[row, col, 1, 0] = M[row, col, 0, 1]
