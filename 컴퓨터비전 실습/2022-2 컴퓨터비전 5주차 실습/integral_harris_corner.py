...
def harris_detector(src, k = 0.04, threshold_rate = 0.01, fsize=5):
    ...
    for row in range(h):
        for col in range(w):
            ...
            # trace_M 계산
            # R 계산 Harris 방정식 구현
            # M_harris를 활용하세요. M[row, col, 0, 0]은 Ix**2를 필터링한 row, col의 픽셀값을 의미합니다. 
            ##########################################################################
...
def calc_M_integral(IxIx_integral, IxIy_integral, IyIy_integral, fsize=5):
    ...
    for row in range(h):
        for col in range(w):
            M[row, col, 0, 0] = calc_local_integral_value(IxIx_integral_pad, left_top=(row, col), right_bottom=(row + fsize - 1, col + fsize - 1))
            M[row, col, 0, 1] = calc_local_integral_value(IxIy_integral_pad, left_top=(row, col), right_bottom=(row + fsize - 1, col + fsize - 1))
            M[row, col, 1, 1] = calc_local_integral_value(IyIy_integral_pad, left_top=(row, col), right_bottom=(row + fsize - 1, col + fsize - 1))
            M[row, col, 1, 0] = M[row, col, 0, 1]
...
def main():
    ...
    cv2.imshow('harris_img ' + "2084033" , harris_img)
    cv2.imshow('harris_integral_img ' + "2084033", harris_integral_img)
    cv2.waitKey()
